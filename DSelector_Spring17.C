#include "DSelector_Spring17.h"
#include <TEnv.h>
#include <sstream>

void DSelector_Spring17::Init(TTree *locTree)
{
	...
	dOutputFileName = "Spring17.root"; //"" for none
	...
	//EXAMPLE MANUAL HISTOGRAMS:
	 ...
	 string dOutputFileNameSt = dOutputFileName;
	 const char *efile = "Efiles/Spring17Efile";

	 fp = fopen(efile,"r");
	 for(nbins=1;nbins<285;nbins++){
	   if(fscanf(fp,"%lg",&ecentreArray[nbins])==EOF) break; //fills ecentreArray with data from efile.
	 }
	 fclose(fp);
	 ...
	 dHist_BeamEnergy = new TH1I("BeamEnergy", ";Beam Energy (GeV)", 275, ebinArray);
...
Bool_t DSelector_Spring17::Process(Long64_t locEntry)
{
	...
	*/
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		*/
		...
		//	Hist_event_BeamBunch->Fill(DT_RF, 0);  //Need this for enh hist calculations
		//Hist_event_BeamBunch->Fill(DT_RF);
		Double_t iIsPrompt_Measured=0;
	
		if(( DT_RF> -18.036) && ( DT_RF< -2.004)){
		        iIsPrompt_Measured = 2;
			//	Hist_event_BeamBunch->Fill(DT_RF,-0.125); 
		}
		else if((DT_RF >= -2.004) && ( DT_RF<= 2.004)){
		  	iIsPrompt_Measured = 1;
			//Hist_event_BeamBunch->Fill(DT_RF,1);
		}
		else if((DT_RF > 2.004) && (DT_RF < 18.036)){
			iIsPrompt_Measured = 2;
			//	Hist_event_BeamBunch->Fill(DT_RF,-0.125); 
		}
		else
		continue;//Need this block for enh hist calculations
		...
		if(locUsedSoFar_BeamEnergy.find(locBeamID) == locUsedSoFar_BeamEnergy.end())
		{
		  //dHist_BeamEnergy->Fill(locBeamP4.E());
		        Fill1DHisto(dHist_BeamEnergy,locBeamP4.E(),iIsPrompt_Measured); 
...
void DSelector_Spring17::Finalize(void)
{
